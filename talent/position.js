...
fs.createReadStream("position.csv")
...
async function insertNewState(data,client) {
  const text = 'INSERT INTO position (id, position_name,function_id,created_at,updated_at) VALUES ($1, $2,$3,$4,$5)'

  const queryTotalState = "SELECT COUNT(*) FROM position";
  ...
  for (let row of data) {
    ...
    if(row[0]!=''){
      ...
      if(row[0]!=''){
        const findRecord = "SELECT COUNT(*) FROM position WHERE position_name='"+row[0]+"'";
        ...
        if(result1.rows[0].count==0){
          console.log("found and insert " + result1.rows[0].count + " row:", row[0] +" "+insertCount);
            ...
            // async/await
            try {
              const values = [insertCount++,row[0],null,now,now]
