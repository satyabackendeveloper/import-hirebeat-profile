...
fs.createReadStream("Company_12_13_2022_GMO_93K.csv")
...
async function insertNewState(data,client) {
  const query_product = 'INSERT INTO company_location (id, company_id, location_id, is_hq, created_at, updated_at ) VALUES ($1, $2,$3,$4,$5,$6) RETURNING id';
  
  const queryTotalLocation = "SELECT id FROM company_location ORDER BY id DESC LIMIT 1";
  const totalLocation = await client.query(queryTotalLocation);
  //console.log(totalState.rows[0].id)

  var insertCmpLocation = 0;
  insertCmpLocation++;
